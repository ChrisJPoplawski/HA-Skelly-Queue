...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    async def _play_loop():
        ...
        try:
            while queue:
                ...
                if not ok:
                    break
                await asyncio.sleep(10)  # TODO: replace with notify
                queue.popleft()
    ...
    # Detect/dump services (handy for troubleshooting)
    ...
    async def svc_detect_chars(_):
        ...
        new = dict(entry.data)
        new[CONF_PLAY_CHAR] = play_char
        if cmd_char: new[CONF_CMD_CHAR] = cmd_char
    ...
    async def svc_enqueue(call):
        ...
        if not fn: return
        queue.append(fn); hass.states.async_set(f"{DOMAIN}.queue_length", len(queue))
    ...
    async def svc_enqueue_url(call):
        ...
        queue.append(rel); hass.states.async_set(f"{DOMAIN}.queue_length", len(queue))
        if not is_playing: hass.async_create_task(_play_loop())
    ...
    # helpers visible as states for sensors
    ...
    _LOGGER.info("Skelly Queue v0.3.0 ready for %s", address)
    return True
